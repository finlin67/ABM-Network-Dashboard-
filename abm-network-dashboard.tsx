...
export default function ABMNetworkDashboard() {
  const total = useCounter(1240);
  const engaged = useCounter(856);
  const pipeline = useCounter(214);

  return (
    <div className="w-full h-full flex items-center justify-center bg-background-dark overflow-hidden">
      {/* Container - Responsive with max constraints */}
      <div className="w-full max-w-[600px] aspect-square max-h-[600px] bg-background-dark relative flex flex-col shadow-2xl font-display selection:bg-primary/30 rounded-xl border border-white/5">
        
        {/* Background Patterns */}
        <div className="absolute inset-0 opacity-20 pointer-events-none rounded-xl overflow-hidden" style={{ backgroundImage: 'radial-gradient(#60a5fa 1px, transparent 1px)', backgroundSize: '24px 24px' }} />
        <div className="absolute top-0 right-0 w-[66%] h-[66%] bg-primary/5 rounded-full blur-[100px] -translate-y-1/2 translate-x-1/2 pointer-events-none" />
        
        {/* Header */}
        <header className="h-14 shrink-0 flex items-center justify-between px-6 border-b border-white/5 bg-background-dark/80 backdrop-blur-md z-30 rounded-t-xl">
        ...
        <div className="flex-1 p-6 flex flex-col gap-4 z-20 overflow-hidden">
          ...
          {/* Top Section: Visualization & Key Metrics - Percentage height for scaling */}
          <div className="h-[46%] flex gap-4">
            ...
            {/* Key Stats Column - Percentage width for scaling */}
